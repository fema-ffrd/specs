...
if [ $# -eq 1 ]; then
    ...
    GEOMETRY_ID=$(echo "$CONFIG_JSON_STR" | jq -r '.ras_schema.geometry_id')
else
    MODELDIR=$1
    MODEL=$2
    PLAN_ID=$3
...
GEOINDEX_NUM=$(printf "%s" "$GEOMETRY_ID" | sed 's/[^0-9]//g' | sed 's/^0*//')
if [ -z "$GEOINDEX_NUM" ]; then
    GEOINDEX_NUM=0
fi
GEOINDEX_PADDED=$(printf "%02d" "$GEOINDEX_NUM")
...
python3 /sim/delete_hdf5_group.py ${MODEL}.p${INDEX_PADDED}.hdf

mv "${MODEL}.p${INDEX_PADDED}.hdf" "${MODEL}.p${INDEX_PADDED}.tmp.hdf"

set -o pipefail
if RasUnsteady "${MODEL}.p${INDEX_PADDED}.tmp.hdf" "x${GEOINDEX_PADDED}" 2>&1 | tee /dev/stderr | grep -q "Finished Unsteady Flow Simulation"; then
    sleep 2
