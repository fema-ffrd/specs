IMAGE=hms
docker build -f Dockerfile_4_12 --platform linux/amd64 -t $IMAGE .
...
docker run --platform linux/amd64 --rm -v ./data:/mnt $IMAGE "$(cat examples/hms-simulation-payload-FS.json)"

## Uncomment to run with S3 environment variables where 1) data is uploaded to S3 and 2) results are copied locally
# docker run --platform linux/amd64 --rm --env-file .env -v ./data:/mnt  $IMAGE "$(cat examples/hms-simulation-payload-S3.json)"